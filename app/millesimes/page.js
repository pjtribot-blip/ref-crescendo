...
const DESCRIPTIONS = {
  2025: "Cinquième édition, marquée par la création du Prix Bernadette Beyne et Michelle Debra. Album de l'année : Berio et Žuraj par Sir Simon Rattle.",
        ...
        {editions.map((ed) => (
          <a key={ed.annee} href={`/millesimes/${ed.annee}`} className="block bg-white border border-stone-200 rounded-2xl p-8 hover:border-amber-400 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between gap-6">
              ...
              <div className="text-3xl text-stone-300 group-hover:text-amber-600 transition-colors shrink-0">→</div>
            </div>
          </a>
        ))}
      </div>
